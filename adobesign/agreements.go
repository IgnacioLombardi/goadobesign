...
type signingUrlInfos struct {
	signingUrlSetInfos []signingUrlInfo `json:"signingUrlSetInfos"`
}
type signingUrlInfo struct {
	signingUrls []signingurl `json:"signingUrls"`
	signingUrlSetName string `json:"signingUrlSetName,omitempty"`
}
type signingurl struct {
	email string `json:"email,omitempty"`
	esignUrl string `json:"esignUrl,omitempty"`
}
/********/
...
// GetAgreementUrl retrieves an existing Adobe Sign Agreement e-sign URL
// ref: https://secure.na1.echosign.com/public/docs/restapi/v6#!/agreements/getSigningUrl
func (s *AgreementService) GetAgreementUrl(ctx context.Context, agreementId string) ([]byte, error) {

	u := fmt.Sprintf("%s/%s/signingUrls", agreementsPath, agreementId)
